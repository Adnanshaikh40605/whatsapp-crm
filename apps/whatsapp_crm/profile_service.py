...
class MetaBusinessProfileService:
    ...
    def update_profile(self, payload: dict, logo_path: str | None = None) -> dict:
        ...
        if payload.get("description") is not None:
            body["description"] = str(payload["description"])[:256]
        if payload.get("address") is not None:
            body["address"] = str(payload["address"])[:256]
        if payload.get("email") is not None:
            body["email"] = str(payload["email"]).strip()
        if payload.get("websites") is not None:
            sites = [s.strip() for s in payload["websites"] if s and str(s).strip()][:2]
            body["websites"] = sites
        if payload.get("vertical") is not None:
            body["vertical"] = payload["vertical"]

        if logo_path:
            handle_result = self._upload_profile_picture_handle(logo_path)
