...
| Field | Value |
|-------|--------|
| Name | `pest_ecard_utility` (use this — Meta blocked recreating `pest_ecard_tracked` as Utility) |
| Language | `en_US` |
| Call Now | `+918080748282` (static) |
| E-Brochure URL | `https://api.driveronhire.ai/r/{{1}}` (dynamic) |

Current `pest_business_details` has a **static** URL → cannot pass per-client suffix. Use `pest_ecard_utility` after Meta approves it.
...
Until `pest_ecard_utility` is **APPROVED**, keep using current send without `track_ecard` (no per-click identity).
...
> Run `setup_pest_ecard_tracked` and wait for Meta approval of `pest_ecard_utility` (URL `https://api.driveronhire.ai/r/{{1}}`).

### To Pest Control CRM developer

> On Inquiry **Pest-Card WhatsApp Track** send, call template API with `track_ecard: true`, `template_name: "pest_ecard_utility"`, `customer_name`, `external_id` (inquiry id).  
